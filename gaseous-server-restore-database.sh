...
for entry in $(docker container exec "$GASEOUS_SERVER_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec "$GASEOUS_SERVER_BACKUPS_CONTAINER" sh -c "mariadb -h mariadb -u $GASEOUS_SERVER_DB_USER --password=$MARIADB_PASSWORD -e 'DROP DATABASE $GASEOUS_SERVER_DB_NAME; CREATE DATABASE $GASEOUS_SERVER_DB_NAME;' \
&& gunzip -c ${BACKUP_PATH}${SELECTED_DATABASE_BACKUP} | mariadb -h mariadb -u $GASEOUS_SERVER_DB_USER --password=$MARIADB_PASSWORD $GASEOUS_SERVER_DB_NAME"
