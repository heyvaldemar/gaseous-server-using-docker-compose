#!/usr/bin/env bash
# gaseous-server-restore-database.sh [backup-file-name]
#
# Replaces the Gaseous database with one of the dumps the backups service wrote.
#
#   ./gaseous-server-restore-database.sh               list the dumps and ask
#   ./gaseous-server-restore-database.sh <file-name>   restore that one
#
...
# said /srv/gaseous-server-mariadb/backups while the stack wrote to
# /srv/gaseous-mariadb/backups: on the day it was needed it would have listed
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than gaseous.
set -Eeuo pipefail
...
DB_USER="$(env_of GASEOUS_SERVER_DB_USER)"; DB_PASS="$(env_of GASEOUS_SERVER_DB_PASSWORD)"
...
if ! docker exec -e MYSQL_PWD="$DB_PASS" "$BKP" bash -c "set -euo pipefail
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Gaseous." >&2
  exit 1
fi
