...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14884/badge)](https://www.bestpractices.dev/projects/14884)
...
Every release from v1.2.1 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `gaseous-server-using-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `gaseous-server-using-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `gaseous-server-using-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob gaseous-server-using-docker-compose-<tag>.tar.gz \
  --bundle gaseous-server-using-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/gaseous-server-using-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact gaseous-server-using-docker-compose-<tag>.tar.gz \
  --provenance-path gaseous-server-using-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/gaseous-server-using-docker-compose
```
