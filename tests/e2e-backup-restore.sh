#!/bin/bash
# End-to-end tests for the gaseous-server-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-gaseous}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-gaseous-server-traefik-letsencrypt-docker-compose.yml}"
...
: "${MARIADB_BACKUPS_PATH:=/srv/gaseous-mariadb/backups}"
: "${MARIADB_BACKUP_NAME:=gaseous-mariadb-backup}"
: "${BACKUP_INTERVAL:=24h}"
: "${GASEOUS_SERVER_DB_USER:=gaseousdbuser}"
: "${GASEOUS_SERVER_DB_PASSWORD:?set in .env}"
: "${DATA_BACKUPS_PATH:=/srv/gaseous-application-data/backups}"
: "${DATA_BACKUP_NAME:=gaseous-application-data-backup}"

BACKUPS_PATH="${MARIADB_BACKUPS_PATH%/}"
BACKUP_PREFIX="${MARIADB_BACKUP_NAME}"
BACKUP_EXT=".gz"
INTERVAL="${BACKUP_INTERVAL}"
DB_USER="${GASEOUS_SERVER_DB_USER}"
DB_PASS="${GASEOUS_SERVER_DB_PASSWORD}"
DB_NAME="gaseous"
DB_HOST="mariadb"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./gaseous-server-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./gaseous-server-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$first") with ./gaseous-server-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./gaseous-server-restore-application-data.sh "$(basename "$first")" \
    || { fail "the shipped restore script failed"; return 1; }
